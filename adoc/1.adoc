...
While we're at it, we'll be using the Gradle Kotlin DSL (if you haven't used it already, now's you're chance!).
...
Finally, select where the code rests, click finish, and enjoy you're newly created multiplatform projecJe prent :)
...
* The *Kotlin/JVM* compiler, that compiles to JVM bytecode, which can be used on a JVM or on Android.
...
When using Kotlin/Multiplatform to compile for multiple targets, it helps to understand which compiler is compiling for which target, as they each get their specific configuration.
...
NOTE: We will see later how to also target iregular Phones
...
This code adds the native target corresponding to your host OS, and configure its sourceset to use the same source directory (`src/hostMain/kotlin`) whatever the target.


=== Understanding sourcesets
...
These are the sourceSets we have defined for our project :
...
Let's add the configuration of the source set we created:
...
NOTE: We have not configured the native target source sets because there's no need to right now.
...
image:1-2.png[source directory structure]
